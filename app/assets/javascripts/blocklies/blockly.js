 ...
 * @private
 */
Code.TABS_ = ['blocks', 'javascript', 'xml', 'ruby'];
...
Code.renderContent = function() {
  ...
  } else if (content.id == 'content_ruby') {
    code = Blockly.Ruby.workspaceToCode(Code.workspace);
    content.textContent = code;
    if (typeof PR.prettyPrintOne == 'function') {
      code = content.textContent;
      code = PR.prettyPrintOne(code, 'ruby');
      content.innerHTML = code;
    }
  }
};
...
Code.runJS = function() {
  ...
  var placeonplayer = function(material, playername, x, y, z, moveplayer){
    if (material == "wool:random") {
      material = getRandomWool()
    }

    var code = '/placeonplayer ' + material + ' ' + playername + ' ' + x + ' ' + y + ' ' + z + ' ' + moveplayer;
    codes.push(code);
  }
  var createcube = function(material, playername, x1, y1, z1, x2, y2, z2){
    if (material == "wool:random") {
      material = getRandomWool()
    }

    var code = '/createcube ' + material + ' ' + playername + ' ' + x1 + ' ' + y1 + ' ' + z1 + ' ' + x2 + ' ' + y2 + ' ' + z2;
    codes.push(code);
  }
  ...
  var spawnEntity = function(x, y, z, name, actionsFunc) {
    ...
    if (actionsFunc.when_touched) {
      actions_string = "";
      actionsFunc.when_touched();
      when_touched = "[\"when_punched\"]={[\"actions\"]={" + actions_string + "}},";
    }
    if (actionsFunc.when_used) {
      actions_string = "";
      actionsFunc.when_used();
      when_used = "[\"when_rightclicked\"]={[\"actions\"]={" + actions_string + "}},";
    }

    actions_string = "{" + when_spawned + when_touched + when_used + "}";
    codes.push(
